...
一开始我想的是排序的每一步都去 setState 更新.

在事件处理中,多个 setState 会合并(batch update)

最后用了`await sleep(600)`延迟触发每一步的 setState

React 的新 API: `useTransition`用来执行低优先级的任务.
