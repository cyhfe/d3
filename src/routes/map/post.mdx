...
useEffect(() => {
  async function getData() {
    ...
    const laureates = (await d3.json("data/map/laureates.json")) as Laureates[];
...
- GeoJSON 是一种基于 JSON（JavaScript Object Notation）的标准，用于表示地理空间数据。它可以描述点、线、多边形等几何图形，还可以用于表示属性数据。GeoJSON 格式被广泛应用于 WebGIS、移动地图、空间数据交换等领域。

- TopoJSON 是一个更高效的地理空间数据格式。与 GeoJSON 不同，TopoJSON 使用拓扑结构来描述地理空间数据，这意味着它可以避免数据中的重复点和线段。这种优化使得 TopoJSON 比 GeoJSON 文件更小，从而提高了数据传输和处理效率。TopoJSON 适用于大规模地图数据和互联网地图应用，它可以减少数据传输的时间和成本。

这里使用的是`GeoJSON`

1. 确定 projection(用于将地理空间数据投影到平面或球面上),跟 `scale`同理,传入经纬度返回对应 svg 位置.
